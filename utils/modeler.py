...
class LRStats(object):
    ...
    def reseprint(self):
        eprint("                          Model Fit Statistics ")
        eprint("==============================================================================")
        eprint("AIC     :         %s              BIC   :        %s    " % (self.aic, self.bic))
        eprint("-2Logl  :         %s              SC    :        %s    " % (self.logl, self.sc))
        eprint("==============================================================================")
...
class Checkio(object):
    ...
    def eprint_enter(self):
        eprint("              Analysis of Variables Eligible for Entry  ")
        eprint("==============================================================================")
        eprint("\t%5s\t \t%5s\t \t%5s\t" % ("variable", "Wald Chi-square", "Pr>ChiSq"))
        for i, v in enumerate(self.xwait):
            eprint("    \t%5s\t             \t%10s\t     \t%10s\t" % (v, self.score[i], self.pvalue[i]))
        eprint(" ")

    def eprint_remove(self):
        eprint("              Analysis of Variables Eligible for Remove  ")
        eprint("==============================================================================")
        eprint("\t%5s\t \t%5s\t \t%5s\t" % ("variable", "Wald Chi-square", "Pr>ChiSq"))
        for i, v in enumerate(self.xwait):
            eprint("    \t%5s\t             \t%10s\t     \t%10s\t" % (v, self.score[i], self.pvalue[i]))
...
class GlobalNullTest(object):
    ...
    def score(self):
        ...
        #        eprint(self.x.shape,type(self.x))
        #        eprint(self.y.shape,type(self.y))
        #        eprint(pi_value.shape,type(pi_value))
...
class StepwiseModel(object):
    ...
    def stepwise(self):
        ...
        eprint(logit_res.summary(), '\n')
        history = {}
        history['const'] = LRStats(step, n, 1, logit_res)
        history['const'].reseprint()
        newx = self.X['const']
        for i in np.arange(p):
            ...
            Checkio(xwait, score, pvalue).eprint_enter()  # 打印运行信息
            if (min(pvalue) <= self.slentry):
                ...
                xwait.remove(xin)
                step += 1
                eprint("** step %s: %s entered:\n" % (step, xin))
                eprint(logit_res[xin].summary())
                Beta0 = list(logit_res[xin].params)
                history[xin].reseprint()
                pouttest = history[xin].pchi2[1:]
                waldouttest = history[xin].wald_chi[1:]  # step backward 剔除变量检验
                xouttest = xenter[1:]
                Checkio(xouttest, waldouttest, pouttest).eprint_remove()
                while 1:
                ...
                eprint("    No additional Variables met the %s significance level for entry into the model" % (
                    self.slentry))
                break
            if rb == 1:
                break
